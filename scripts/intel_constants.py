...
parser.add_argument('--num_PLRAM', type=int, default=1)
parser.add_argument('--num_DDR', type=int, default=1)
...
########### original ############
data_size_PLRAM = [8] * 8
table_size_PLRAM = [10000] * 8

data_size_DDR =  [4] * 16 + [8] * 20 + [16] * 1 + [16] * 1 + [32] * 1
table_size_DDR = [100] * 4 + [500] * 2 + [1000] * 10 + [3000] * 1 + [10000] * 7 \
...
print("Total vector length: {}".format(np.sum(data_size_PLRAM + data_size_DDR)))
...
#define TABLE_NUM {}
""".format(FLAGS.AXI_WIDTH_PLRAM, FLAGS.AXI_WIDTH_DDR, 
    FLAGS.int_size, int(FLAGS.AXI_WIDTH_PLRAM / FLAGS.int_size), 
    int(FLAGS.AXI_WIDTH_DDR / FLAGS.int_size),
    FLAGS.num_PLRAM, FLAGS.num_DDR, 
    total_num_PLRAM + total_num_DDR)

    out_str += """
#define TABLE_NUM_DDR {}
#define TABLE_NUM_PLRAM {}
""".format(len(data_size_DDR), len(data_size_PLRAM))

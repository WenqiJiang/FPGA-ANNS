...
#pragma HLS pipeline II=1
    ...
    int cnt_nprobe_per_query = 0;
    int accumulated_scanned_entries_per_query = 0;

    do {

        if ((!s_scanned_cell_id_Input.empty()) && (cnt_nprobe_per_query < nprobe)) {
            ...
            cnt_nprobe_per_query++;
        }

        if (cnt_nprobe_per_query == nprobe) {
            ...
            cnt_nprobe_per_query = 0;
            accumulated_scanned_entries_per_query = 0;
            processed_query_num++;
        }

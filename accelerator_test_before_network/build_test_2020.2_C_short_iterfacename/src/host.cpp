...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    ...
    // size = 101841920
    size_t HBM_embedding0_size =  HBM_embedding0_len * sizeof(float);
    size_t HBM_embedding1_size =  HBM_embedding1_len * sizeof(float);
    size_t HBM_embedding2_size =  HBM_embedding2_len * sizeof(float);
    size_t HBM_embedding3_size =  HBM_embedding3_len * sizeof(float);
    size_t HBM_embedding4_size =  HBM_embedding4_len * sizeof(float);
    size_t HBM_embedding5_size =  HBM_embedding5_len * sizeof(float);
    size_t HBM_embedding6_size =  HBM_embedding6_len * sizeof(float);
    size_t HBM_embedding7_size =  HBM_embedding7_len * sizeof(float);
    size_t HBM_embedding8_size =  HBM_embedding8_len * sizeof(float);
    size_t HBM_embedding9_size =  HBM_embedding9_len * sizeof(float);
    // size_t HBM_embedding10_size =  HBM_embedding10_len * sizeof(float);
    // size_t HBM_embedding11_size =  HBM_embedding11_len * sizeof(float);
    // size_t HBM_embedding12_size =  HBM_embedding12_len * sizeof(float);
    // size_t HBM_embedding13_size =  HBM_embedding13_len * sizeof(float);
    // size_t HBM_embedding14_size =  HBM_embedding14_len * sizeof(float);
    // size_t HBM_embedding15_size =  HBM_embedding15_len * sizeof(float);
    // size_t HBM_embedding16_size =  HBM_embedding16_len * sizeof(float);
    // size_t HBM_embedding17_size =  HBM_embedding17_len * sizeof(float);
    // size_t HBM_embedding18_size =  HBM_embedding18_len * sizeof(float);
    // size_t HBM_embedding19_size =  HBM_embedding19_len * sizeof(float);
    // size_t HBM_embedding20_size =  HBM_embedding20_len * sizeof(float);
    // size_t HBM_embedding21_size =  HBM_embedding21_len * sizeof(float);
    // size_t HBM_embedding22_size =  HBM_embedding22_len * sizeof(float);
    // size_t HBM_embedding23_size =  HBM_embedding23_len * sizeof(float);
    // size_t HBM_embedding24_size =  HBM_embedding24_len * sizeof(float);
    // size_t HBM_embedding25_size =  HBM_embedding25_len * sizeof(float);
    // size_t HBM_embedding26_size =  HBM_embedding26_len * sizeof(float);
    ...
    size_t HBM_out_size = HBM_out_len * sizeof(float); 
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    std::vector<float, aligned_allocator<float>> HBM_embedding0(HBM_embedding0_len, 0);
    std::vector<float, aligned_allocator<float>> HBM_embedding1(HBM_embedding1_len, 0);
    std::vector<float, aligned_allocator<float>> HBM_embedding2(HBM_embedding2_len, 0);
    std::vector<float, aligned_allocator<float>> HBM_embedding3(HBM_embedding3_len, 0);
    std::vector<float, aligned_allocator<float>> HBM_embedding4(HBM_embedding4_len, 0);
    std::vector<float, aligned_allocator<float>> HBM_embedding5(HBM_embedding5_len, 0);
    std::vector<float, aligned_allocator<float>> HBM_embedding6(HBM_embedding6_len, 0);
    std::vector<float, aligned_allocator<float>> HBM_embedding7(HBM_embedding7_len, 0);
    std::vector<float, aligned_allocator<float>> HBM_embedding8(HBM_embedding8_len, 0);
    std::vector<float, aligned_allocator<float>> HBM_embedding9(HBM_embedding9_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding10(HBM_embedding10_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding11(HBM_embedding11_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding12(HBM_embedding12_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding13(HBM_embedding13_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding14(HBM_embedding14_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding15(HBM_embedding15_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding16(HBM_embedding16_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding17(HBM_embedding17_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding18(HBM_embedding18_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding19(HBM_embedding19_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding20(HBM_embedding20_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding21(HBM_embedding21_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding22(HBM_embedding22_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding23(HBM_embedding23_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding24(HBM_embedding24_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding25(HBM_embedding25_len, 0);
    // std::vector<float, aligned_allocator<float>> HBM_embedding26(HBM_embedding26_len, 0);
    ...
    std::vector<float, aligned_allocator<float>> HBM_out(HBM_out_len, 0);
...
// ============================================================================	
    ...
    OCL_CHECK(err, cl::Buffer buffer_HBM_query_vectors(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY | CL_MEM_EXT_PTR_XILINX, 
            HBM_query_vector_size, HBM_query_vectors.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_HBM_vector_quantizer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY | CL_MEM_EXT_PTR_XILINX, 

...
#pragma HLS pipeline II=1
                s_partial_cell_distance_out.write(s_partial_cell_distance_in.read());
            }
...
template<const int query_num, const int centroids_per_partition, const int total_centriods>
void compute_cell_distance_head(
    const int systolic_array_id,
    hls::stream<float>& s_centroid_vectors_in,
    hls::stream<float>& s_centroid_vectors_out,
    hls::stream<float>& s_query_vectors_in,
...
#pragma HLS stream variable=s_partial_cell_distance_forward depth=8
    ...
    // head
    compute_cell_distance_head<QUERY_NUM, CENTROIDS_PER_PARTITION_EVEN, NLIST>(
        0,
        s_centroid_vectors,
...
#pragma HLS UNROLL
        compute_cell_distance_middle<QUERY_NUM, CENTROIDS_PER_PARTITION_EVEN, NLIST>(
            s,
    ...
    // tail
    compute_cell_distance_tail<QUERY_NUM, CENTROIDS_PER_PARTITION_EVEN, CENTROIDS_PER_PARTITION_LAST_PE, NLIST>(

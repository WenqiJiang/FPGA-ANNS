...
void replicate_s_scanned_entries_every_cell_PQ_lookup_computation(
    hls::stream<int>& s_scanned_entries_every_cell_PQ_lookup_computation,
    hls::stream<int> (&s_scanned_entries_every_cell_PQ_lookup_computation_replicated)[HBM_CHANNEL_NUM]);

template<const int query_num, const int nprobe>
void send_s_last_element_valid_PQ_lookup_computation(
    hls::stream<int>& s_last_valid_channel,
    hls::stream<int> (&s_last_element_valid_PQ_lookup_computation)[HBM_CHANNEL_NUM]);
...
#pragma HLS resource variable=distance_LUT core=RAM_2P_BRAM
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
        ...
        for (int nprobe_id = 0; nprobe_id < nprobe; nprobe_id++) {
            ...
            int last_element_valid_0 = 
                s_last_element_valid_PQ_lookup_computation.read();
            int last_element_valid_1 = 
                s_last_element_valid_PQ_lookup_computation.read();
            int last_element_valid_2 = 
                s_last_element_valid_PQ_lookup_computation.read();
...
#pragma HLS pipeline II=1
                ...
                if (entry_id >= scanned_entries_every_cell - 3) {
                    if (
                        ((entry_id == (scanned_entries_every_cell - 3)) && (last_element_valid_0 == 0)) ||
                        ((entry_id == (scanned_entries_every_cell - 2)) && (last_element_valid_1 == 0)) ||
                        ((entry_id == (scanned_entries_every_cell - 1)) && (last_element_valid_2 == 0))) {
                        out.vec_ID = -1;
                        out.dist = LARGE_NUM;
                    }
                }
...
void send_s_last_element_valid_PQ_lookup_computation(
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
        ...
        for (int nprobe_id = 0; nprobe_id < nprobe; nprobe_id++) {
            ...
            for (int s = 0; s < HBM_CHANNEL_NUM; s++) {
#pragma HLS UNROLL
                // each compute unit is connected to 1 HBM channels, 
                //   which has 3 PQ codes in the last 3 entries, 
                //   send 3 signals to judge if they are valid
                for (int c = 0; c < 3; c++) {

                    if (s * 3 + c <= last_valid_channel) {
...
#pragma HLS inline
    ...
    hls::stream<distance_LUT_PQ16_t> s_distance_LUT_systolic[HBM_CHANNEL_NUM];
...
// #pragma HLS RESOURCE variable=s_last_element_valid_PQ_lookup_computation core=FIFO_SRL
    ...
    // remaining PEs
    for (int j = 1; j < HBM_CHANNEL_NUM; j++) {
#pragma HLS UNROLL
        PQ_lookup_computation<query_num, nprobe>(
            // input streams
    ...
    dummy_distance_LUT_consumer<query_num, nprobe>(
        s_distance_LUT_systolic[HBM_CHANNEL_NUM - 1]);
}

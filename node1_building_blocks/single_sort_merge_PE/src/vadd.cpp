...
#pragma HLS stream variable=s_merge_output depth=8
    ...
    broadcast_stream<MERGE_ITER * MERGE_LEN, PE_NUM_DIST_COMP, QUERY_NUM>(table_DDR0, s_input_stream);

    sort_merge_PE(s_input_stream, s_merge_output);
...
#pragma HLS array_partition variable=input_array cyclic factor=stream_num
    ...
    for (int iter = 0; iter < query_num; iter++) { 
        ...
        for (int i = 0; i < total_len; i++) {
        #pragma HLS pipeline II=1
            input_array[i] = data_source[query_num * total_len + i];
        }

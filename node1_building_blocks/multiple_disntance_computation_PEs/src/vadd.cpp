...
void broadcast_query_vector(
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
        ...
        #pragma HLS pipeline II=1
            for (int s = 0; s < PE_NUM_DIST_COMP; s++) {
                s_query_vectors[s].write(table_DDR0[query_id * D + d]);
            }
        }
    }
}
...
void broadcast_init_centroid_vectors(
    ...
    ////// TODO: Fix this for real dataset //////
    for (int i = 0; i < CENTROIDS_PER_PARTITION * D; i++) {
        float reg = table_DDR1[i];
        for (int s = 0; s < PE_NUM_DIST_COMP; s++) {
            #pragma HLS UNROLL
            s_centroid_vectors[s].write(reg);
        }
    }
}
...
#pragma HLS inline
    ...
    for (int s = 0; s < PE_NUM_DIST_COMP; s++) {
        #pragma HLS UNROLL
        compute_cell_distance<QUERY_NUM>(s_centroid_vectors[s], 
...
#pragma HLS array_partition variable=cell_centroids_partition cyclic factor=8 dim=2 
...
#pragma HLS array_partition variable=local_query_buffer cyclic factor=8
    ...
    for (int c = 0; c < CENTROIDS_PER_PARTITION; c++) {
        for (int d = 0; d < D; d++) {
        #pragma HLS pipeline II=1
            cell_centroids_partition[c][d] = s_centroid_vectors.read();
        }
    }
...
void write_result(
    hls::stream<float> (&s_result)[PE_NUM_DIST_COMP], float* results_out) {
    
    for (int i = 0; i < total_len; i++) {
        float reg = 
            s_result[0].read() + s_result[1].read() + 
            s_result[2].read() + s_result[3].read() + 
            s_result[4].read() + s_result[5].read() + 
            s_result[6].read() + s_result[7].read() + 
            s_result[8].read() + s_result[9].read() + 
            s_result[10].read() + s_result[11].read() + 
            s_result[12].read() + s_result[13].read() + 
            s_result[14].read() + s_result[15].read() + 
            s_result[16].read() + s_result[17].read() + 
            s_result[18].read() + s_result[19].read() + 
            s_result[20].read() + s_result[21].read() + 
            s_result[22].read() + s_result[23].read() + 
            s_result[24].read() + s_result[25].read() + 
            s_result[26].read() + s_result[27].read() + 
            s_result[28].read() + s_result[29].read() + 
            s_result[30].read() + s_result[31].read();
        results_out[i] = reg;
    }
}

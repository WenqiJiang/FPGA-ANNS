...
// OPENCL HOST CODE AREA END
    ...
    for (int query_id = 0 ; query_id < (QUERY_NUM < 64? QUERY_NUM: 64); query_id++) {
        ...
        for (int i = 0; i < NPROBE / 16; i++) {
            int result_adddr_hw = query_id * (NPROBE * K + NPROBE / 16) + i;
            int result_adddr_sw = query_id * (NPROBE / 16) + i;
            for (int j = 0; j < 16; j++) {
                ap_uint<32> tmp_hw = source_hw_results[result_adddr_hw].range((j + 1) * 32 - 1, j * 32);
                ap_uint<32> tmp_sw = source_sw_results_cell_IDs[result_adddr_sw].range((j + 1) * 32 - 1, j * 32);
                int cell_ID_hw = *((int*) (&tmp_hw));
                int cell_ID_sw = *((int*) (&tmp_sw));
                printf("nprobe_id %d\thw: %d\tsw: %d\tTrue? %d\n", 
                    i * 16 + j, cell_ID_hw, cell_ID_sw, cell_ID_sw == cell_ID_hw? true : false);
                if (match) {
                    match = (cell_ID_sw == cell_ID_hw);
                }
            }
        }

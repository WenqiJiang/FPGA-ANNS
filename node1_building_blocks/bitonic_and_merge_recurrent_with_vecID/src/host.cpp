...
//     char* source_sw_results_content_char = (char*) malloc(source_sw_results_content_size);
    ...
    std::ifstream DDR_0_content_fstream(
        "/mnt/scratch/wenqi/FPGA-ANNS/node1_building_blocks/bitonic_and_merge_recurrent_with_vecID/dist_array_float32_8192_raw", 
    ...
    for (int query_id = 0; query_id < QUERY_NUM; query_id++) {
        for (int nprobe_id = 0; nprobe_id < NPROBE; nprobe_id++) {
            std::cout << "query_id = " << query_id 
                << "\tnprobe_id = " << nprobe_id 
                << "\tsw dist = " << source_sw_results_dist[nprobe_id] 
                << "\thw dist = " << source_hw_results[query_id * NPROBE + nprobe_id].dist
                << "\tsw cell_ID = " << source_sw_results_ids[nprobe_id] 
                << "\thw cell_ID = " << source_hw_results[query_id * NPROBE + nprobe_id].cell_ID
                << "\t\t" << ((source_sw_results_dist[nprobe_id] - source_hw_results[query_id * NPROBE + nprobe_id].dist < 0.01) &&
                    (source_sw_results_ids[nprobe_id] == 

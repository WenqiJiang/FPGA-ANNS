...
//// INIT SORTED arrays ////
    ...
    // HBM0: query vector, QUERY_NUM * D
    int HBM_0_content_size = 10000 * 128 * sizeof(float); 
    
    // HBM1: centroids of single PE, CENTROIDS_PER_PARTITION * D
    ...
    for (int query_id = 0; query_id < QUERY_NUM; query_id++) {
        for (int nprobe_id = 0; nprobe_id < NPROBE; nprobe_id++) {
            ...
            std::cout << "query_id = " << query_id 
                ...
                << "\thw ID = " << cell_ID_local
                << "\t\t" << 
                    bool((source_sw_results_dist[query_id * NPROBE + nprobe_id] - dist_local <0.1)&& 
                    (source_sw_results_dist[query_id * NPROBE + nprobe_id] - dist_local > -0.1) && 

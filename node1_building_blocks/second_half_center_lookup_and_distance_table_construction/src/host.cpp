...
//////////////////////////////   TEMPLATE END  //////////////////////////////
    ...
    // HBM 1 -> searched cell ID
    int HBM_1_content_size = QUERY_NUM * 32 * sizeof(int) < 64 * 32 * sizeof(int)? 
        QUERY_NUM * 32 * sizeof(int) : 64 * 32 * sizeof(int);

    // HBM 2 -> coarse center vectors
    int HBM_2_content_size = 8192 * 128 * sizeof(float);

    // HBM 3 -> PQ quantizers
    int HBM_3_content_size = 16 * 256 * 8 * sizeof(float);

    // HBM 27 -> result, float32_batch_size_64_nprobe_32_K_256_M_16_raw
    int source_sw_results_content_size = 
        size_results_out * sizeof(result_t) < 64 * 32 * 256 * 16 * sizeof(float)?
        size_results_out * sizeof(result_t) : 64 * 32 * 256 * 16 * sizeof(float);
    ...
    char* HBM_3_content_char = (char*) malloc(HBM_3_content_size);
    ...
    std::ifstream HBM_2_content_fstream(
        ...
        std::ios::in | std::ios::binary);
    std::ifstream HBM_3_content_fstream(
    ...
    HBM_3_content_fstream.read(HBM_3_content_char, HBM_3_content_size);
    ...
    std::cout << "HBM_3_content_fstream read bytes: " << HBM_3_content_fstream.gcount() << std::endl;
    ...
    memcpy(&HBM_embedding3[0], HBM_3_content_char, HBM_3_content_size);
    ...
    for (int query_id = 0 ; query_id < QUERY_NUM < 64? QUERY_NUM: 64; query_id++) {
        for (int nprobe_id = 0; nprobe_id < NPROBE; nprobe_id++) {
            for (int row = 0; row < K; row++) {
                ...
                for (int j = 0; j < 16; j++) {
                    ap_uint<32> tmp_hw = source_hw_results[result_adddr].range((j + 1) * 32 - 1, j * 32);
                    ap_uint<32> tmp_sw = source_sw_results[result_adddr].range((j + 1) * 32 - 1, j * 32);
                    float dist_hw = *((float*) (&tmp_hw));
                    float dist_sw = *((float*) (&tmp_sw));
                    float delta = dist_hw - dist_sw;
                    printf("query_id %d\tnprobe_id %d\tdist_table(%d, %d)\thw: %f\tsw: %f\tdelta: %f\n", 
                        query_id, nprobe_id, row, j, dist_hw, dist_sw, delta);
                }
            }
        }
    }

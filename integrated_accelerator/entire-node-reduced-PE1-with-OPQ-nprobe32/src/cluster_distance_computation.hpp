...
#pragma HLS UNROLL
        compute_cell_distance<QUERY_NUM, CENTROIDS_PER_PARTITION_EVEN>(
            s * CENTROIDS_PER_PARTITION_EVEN, s_centroid_vectors[s], 
            s_query_vectors[s], s_partial_cell_distance[s]);
    }
    compute_cell_distance<QUERY_NUM, CENTROIDS_PER_PARTITION_LAST_PE>(
        23 * CENTROIDS_PER_PARTITION_EVEN, s_centroid_vectors[23], 
        s_query_vectors[23], s_partial_cell_distance[23]);
}
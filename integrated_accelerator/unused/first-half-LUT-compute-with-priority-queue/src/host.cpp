...
//////////////////////////////   TEMPLATE END  //////////////////////////////
    ...
    // HBM 2 -> PQ quantizers
    int HBM_2_content_size = 16 * 256 * 8 * sizeof(float);
    ...
    char* HBM_2_content_char = (char*) malloc(HBM_2_content_size);
    char* source_sw_results_dist_table_content_char = (char*) malloc(source_sw_results_dist_table_content_size);
    char* source_sw_results_cell_IDs_content_char = (char*) malloc(source_sw_results_cell_IDs_content_size);

    std::ifstream HBM_0_content_fstream(
        "/mnt/scratch/wenqi/FPGA-ANNS/saved_npy_data/query_vectors_float32_10000_128_raw", 
        std::ios::in | std::ios::binary);
    std::ifstream HBM_1_content_fstream(
        "/mnt/scratch/wenqi/FPGA-ANNS/saved_npy_data/coarse_centers_float32_8192_128_raw", 
        std::ios::in | std::ios::binary);
    std::ifstream HBM_2_content_fstream(
    ...
    HBM_0_content_fstream.read(HBM_0_content_char, HBM_0_content_size);
    HBM_1_content_fstream.read(HBM_1_content_char, HBM_1_content_size);
    HBM_2_content_fstream.read(HBM_2_content_char, HBM_2_content_size);
    source_sw_results_dist_table_content_fstream.read(source_sw_results_dist_table_content_char, source_sw_results_dist_table_content_size);
    source_sw_results_cell_IDs_content_fstream.read(source_sw_results_cell_IDs_content_char, source_sw_results_cell_IDs_content_size);
    
    std::cout << "HBM_0_content_fstream read bytes: " << HBM_0_content_fstream.gcount() << std::endl;
    std::cout << "HBM_1_content_fstream read bytes: " << HBM_1_content_fstream.gcount() << std::endl;
    std::cout << "HBM_2_content_fstream read bytes: " << HBM_2_content_fstream.gcount() << std::endl;
    std::cout << "source_sw_results_dist_table_content_fstream read bytes: " << source_sw_results_dist_table_content_fstream.gcount() << std::endl;
    std::cout << "source_sw_results_cell_IDs_content_fstream read bytes: " << source_sw_results_cell_IDs_content_fstream.gcount() << std::endl;

    memcpy(&HBM_embedding0[0], HBM_0_content_char, HBM_0_content_size);
    memcpy(&HBM_embedding1[0], HBM_1_content_char, HBM_1_content_size);
    memcpy(&HBM_embedding2[0], HBM_2_content_char, HBM_2_content_size);
    memcpy(&source_sw_results_dist_table[0], source_sw_results_dist_table_content_char, source_sw_results_dist_table_content_size);
    memcpy(&source_sw_results_cell_IDs[0], source_sw_results_cell_IDs_content_char, source_sw_results_cell_IDs_content_size);

    free(HBM_0_content_char);
    free(HBM_1_content_char);
    free(HBM_2_content_char);
    free(source_sw_results_dist_table_content_char);
    free(source_sw_results_cell_IDs_content_char);
...
//     DDR_embedding1Ext.flags = bank[1 + 32];
    ...
    sourcce_hw_resultsExt.flags = bank[27]; // HBM w27
...
// OPENCL HOST CODE AREA END
    ...
    std::vector<int> hw_result_ID(NPROBE); 
    std::vector<int> sw_result_ID(NPROBE); 


    for (int i = 0; i < NPROBE / 16; i++) {
        
        for (int j = 0; j < 16; j++) {
            ap_uint<32> tmp_hw = source_hw_results[i].range((j + 1) * 32 - 1, j * 32);
            ap_uint<32> tmp_sw = source_sw_results_cell_IDs[i].range((j + 1) * 32 - 1, j * 32);
            int cell_ID_hw = *((int*) (&tmp_hw));
            int cell_ID_sw = *((int*) (&tmp_sw));
            hw_result_ID[i * 16 + j] = cell_ID_hw;
            sw_result_ID[i * 16 + j] = cell_ID_sw;
        }
    }
    std::sort(hw_result_ID.begin(), hw_result_ID.end());
    std::sort(sw_result_ID.begin(), sw_result_ID.end());
    for (int i = 0 ; i < NPROBE; i++) {
        printf("nprobe_id = %d, hw ID = %d, sw ID = %d, delta = %d\n", 
            i, hw_result_ID[i], sw_result_ID[i], hw_result_ID[i] - sw_result_ID[i]);
    }

    // for (int query_id = 0 ; query_id < 1/*(query_num < 64? query_num: 64)*/; query_id++) {


    //     // priority queue will does not sort the distance in the end, so cell_ID in the result
    //     //   is not equal to the CPU version
    //     for (int i = 0; i < NPROBE / 16; i++) {

    //         results_out[i] = s_result_searched_cell_ID.read();       

    //         for (int j = 0; j < 16; j++) {
    //             ap_uint<32> tmp_hw = source_hw_results[result_adddr_hw].range((j + 1) * 32 - 1, j * 32);
    //             ap_uint<32> tmp_sw = source_sw_results_dist_table[result_adddr_sw].range((j + 1) * 32 - 1, j * 32);
    //             float dist_hw = *((float*) (&tmp_hw));
    //     }
    //     source_sw_results_cell_IDs;

    //     printf("query_id: %d\nDistance Tables:\n", query_id);
    //     for (int nprobe_id = 0; nprobe_id < NPROBE; nprobe_id++) {
    //         for (int row = 0; row < K; row++) {
    //             int result_adddr_hw = query_id * (NPROBE * K + NPROBE / 16) + NPROBE / 16 + nprobe_id * K + row;
    //             int result_adddr_sw = query_id * NPROBE * K + nprobe_id * K + row;
    //             for (int j = 0; j < 16; j++) {
    //                 ap_uint<32> tmp_hw = source_hw_results[result_adddr_hw].range((j + 1) * 32 - 1, j * 32);
    //                 ap_uint<32> tmp_sw = source_sw_results_dist_table[result_adddr_sw].range((j + 1) * 32 - 1, j * 32);
    //                 float dist_hw = *((float*) (&tmp_hw));
    //                 float dist_sw = *((float*) (&tmp_sw));
    //                 float delta = dist_hw - dist_sw;
    //                 printf("nprobe_id %d\tdist_table(%d, %d)\thw: %f\tsw: %f\tdelta: %f\n", 
    //                     nprobe_id, row, j, dist_hw, dist_sw, delta);
    //                 if (match) {
    //                     match = (dist_hw - dist_sw < 0.1) && (dist_hw - dist_sw > -0.1);
    //                 }
    //             }
    //         }
    //     }

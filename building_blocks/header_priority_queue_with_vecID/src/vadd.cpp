...
#pragma HLS dataflow
    ...
    Priority_queue<single_PQ_result, PRIORITY_QUEUE_LEN, Collect_smallest> pq;
...
void read_input(const float* data_source, hls::stream<single_PQ_result> &s_input) {
    for (int i = 0; i < total_len; i++) {
#pragma HLS pipeline II=1
        single_PQ_result reg;
        reg.vec_ID = i;
        reg.dist = data_source[i];
        s_input.write(reg);
    }
}

template<const int total_len>
void write_result(hls::stream<single_PQ_result> &s_output, ap_uint<64>* output) {
    for (int i = 0; i < total_len; i++) {
#pragma HLS pipeline II=1
        single_PQ_result reg = s_output.read();
        int vec_ID = reg.vec_ID;
        float dist = reg.dist;
        ap_uint<64> out_reg;
        out_reg(31, 0) = *((ap_uint<32>*) (&vec_ID));
        out_reg(63, 32) = *((ap_uint<32>*) (&dist));
        output[i] = out_reg;
    }
}
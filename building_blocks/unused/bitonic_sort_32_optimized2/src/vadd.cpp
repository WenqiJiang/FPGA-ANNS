...
template<const int total_len, const int partition_num,
    const int elements_per_partition, const int operations_per_partition>
...
// #pragma HLS inline
    ...
    #pragma HLS UNROLL
        for (int j = 0; j < operations_per_partition; j++) {
        #pragma HLS UNROLL
        compare_swap(array, i * elements_per_partition + j, i * elements_per_partition + operations_per_partition + j);
        }
    }
}
...
void bitonic_sort(float input_array[32]) {
    ...
    // const int elements_per_partition = total_len / partition_num;
    // const int operations_per_partition = elements_per_partition / 2;
    compare_swap_range_interval<32, 16, 2, 1>(input_array);

    // Stage 2: 2 -> 4
    compare_swap_range_head_tail<32, 8, 4, 2>(input_array);
    compare_swap_range_interval<32, 16, 2, 1>(input_array);

    // Stage 3: 4 -> 8
    compare_swap_range_head_tail<32, 4, 8, 4>(input_array);
    compare_swap_range_interval<32, 8, 4, 2>(input_array);
    compare_swap_range_interval<32, 16, 2, 1>(input_array);

    // Stage 4: 8 -> 16
    compare_swap_range_head_tail<32, 2, 16, 8>(input_array);
    compare_swap_range_interval<32, 4, 8, 4>(input_array);
    compare_swap_range_interval<32, 8, 4, 2>(input_array);
    compare_swap_range_interval<32, 16, 2, 1>(input_array);
      
    // Stage 5: 16 -> 32
    compare_swap_range_head_tail<32, 1, 32, 16>(input_array);
    compare_swap_range_interval<32, 2, 16, 8>(input_array);
    compare_swap_range_interval<32, 4, 8, 4>(input_array);
    compare_swap_range_interval<32, 8, 4, 2>(input_array);
    compare_swap_range_interval<32, 16, 2, 1>(input_array);
}

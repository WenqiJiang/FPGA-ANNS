...
#pragma HLS array_partition variable=output_stream dim=1
    ...
    sort_and_reduction_recurrent<2, 16>(input_stream, output_stream);
...
#pragma HLS pipeline II=1
            array[i][j] = data_source[i * dim2 + j];
        }
    }
...
#pragma HLS UNROLL
            for (int j = 0; j < dim2; j++) {
#pragma HLS UNROLL
                input_stream[i][j].write(array[i][j]);
            }
        }
    }
}
...
template<>
void sort_and_reduction_recurrent<2, 16>(
    hls::stream<float> (&input_stream)[2][16],
...
#pragma HLS dataflow
    sort_and_reduction<2, 16>(input_stream, sorted_input_stream);

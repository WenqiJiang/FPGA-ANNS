...
#pragma HLS array_partition variable=output_stream dim=1
    ...
    sort_and_reduction_recurrent<128, 16>(input_stream, output_stream);
...
void broadcast_array(
    const float* data_source, hls::stream<float> (&input_stream)[total_len]) {
    
    float array[total_len];
...
template<>
void sort_and_reduction<128, 16>(
    hls::stream<float> (&input_stream)[128],
    hls::stream<float> (&output_stream)[16]) {
// #pragma HLS ARRAY_RESHAPE variable=input_stream block factor=8 dim=1 // [128] -> [8][16]?

    hls::stream<float> input_stream_local[8][16];
#pragma HLS array_partition variable=input_stream_local complete

    hls::stream<float> result_stage_0[8][16];
    hls::stream<float> result_stage_1[4][16];
    hls::stream<float> result_stage_2[2][16];
#pragma HLS array_partition variable=result_stage_0 complete
#pragma HLS array_partition variable=result_stage_1 complete
#pragma HLS array_partition variable=result_stage_2 complete
...
#pragma HLS UNROLL
        for (int j = 0; j < 16; j++) {
#pragma HLS UNROLL
            forward_stream<SORT_ARRAY_NUM>(
                input_stream[i * 16 + j], input_stream_local[i][j]);
        }
    }

    for (int i = 0; i < 8; i++) {
#pragma HLS UNROLL
        bitonic_sort<16>(input_stream_local[i], result_stage_0[i]);
    }
...
#pragma HLS UNROLL
        parallel_merge_sort<16>(
            result_stage_1[2 * i], result_stage_1[2 * i + 1], 
            result_stage_2[i]);
    }
   
    parallel_merge_sort<16>(result_stage_2[0], result_stage_2[1], output_stream); 
}
...
template<>
void sort_and_reduction_recurrent<128, 16>(
    hls::stream<float> (&input_stream)[128],
...
#pragma HLS dataflow
    ...
    sort_and_reduction<128, 16>(input_stream, sorted_input_stream);

    parallel_merge_sort<16>(
        current_smallest_redirectd_stream, sorted_input_stream, 
        current_smallest_sorted_stream); 

    reduction_stream_redirect<16>(
        current_smallest_sorted_stream,
        current_smallest_redirectd_stream,
        final_smallest_stream);
}

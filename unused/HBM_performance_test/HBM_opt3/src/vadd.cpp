...
#pragma HLS dataflow
    ...
    hls::stream<t_axi> s_HBM[28];
#pragma HLS stream variable=s_HBM depth=128
#pragma HLS array_partition variable=s_HBM dim=1
...
#pragma HLS array_partition variable=s_output dim=1
    ...
    load_HBM(table_HBM0, s_HBM[0]);
    load_HBM(table_HBM1, s_HBM[1]);
    load_HBM(table_HBM2, s_HBM[2]);
    load_HBM(table_HBM3, s_HBM[3]);
    load_HBM(table_HBM4, s_HBM[4]);
    load_HBM(table_HBM5, s_HBM[5]);
    load_HBM(table_HBM6, s_HBM[6]);
    load_HBM(table_HBM7, s_HBM[7]);
    load_HBM(table_HBM8, s_HBM[8]);
    load_HBM(table_HBM9, s_HBM[9]);
    load_HBM(table_HBM10, s_HBM[10]);
    load_HBM(table_HBM11, s_HBM[11]);
    load_HBM(table_HBM12, s_HBM[12]);
    load_HBM(table_HBM13, s_HBM[13]);
    load_HBM(table_HBM14, s_HBM[14]);
    load_HBM(table_HBM15, s_HBM[15]);
    load_HBM(table_HBM16, s_HBM[16]);
    load_HBM(table_HBM17, s_HBM[17]);
    load_HBM(table_HBM18, s_HBM[18]);
    load_HBM(table_HBM19, s_HBM[19]);
    load_HBM(table_HBM20, s_HBM[20]);
    load_HBM(table_HBM21, s_HBM[21]);
    load_HBM(table_HBM22, s_HBM[22]);
    load_HBM(table_HBM23, s_HBM[23]);
    load_HBM(table_HBM24, s_HBM[24]);
    load_HBM(table_HBM25, s_HBM[25]);
    load_HBM(table_HBM26, s_HBM[26]);
    load_HBM(table_HBM27, s_HBM[27]);
    // load_HBM(table_HBM28, s_HBM[28]);
    // load_HBM(table_HBM29, s_HBM[29]);
    // load_HBM(table_HBM30, s_HBM[30]);
    // load_HBM(table_HBM31, s_HBM[31]);

    consume(s_HBM[0], s_output[0]);
    consume(s_HBM[1], s_output[1]);
    consume(s_HBM[2], s_output[2]);
    consume(s_HBM[3], s_output[3]);
    consume(s_HBM[4], s_output[4]);
    consume(s_HBM[5], s_output[5]);
    consume(s_HBM[6], s_output[6]);
    consume(s_HBM[7], s_output[7]);
    consume(s_HBM[8], s_output[8]);
    consume(s_HBM[9], s_output[9]);
    consume(s_HBM[10], s_output[10]);
    consume(s_HBM[11], s_output[11]);
    consume(s_HBM[12], s_output[12]);
    consume(s_HBM[13], s_output[13]);
    consume(s_HBM[14], s_output[14]);
    consume(s_HBM[15], s_output[15]);
    consume(s_HBM[16], s_output[16]);
    consume(s_HBM[17], s_output[17]);
    consume(s_HBM[18], s_output[18]);
    consume(s_HBM[19], s_output[19]);
    consume(s_HBM[20], s_output[20]);
    consume(s_HBM[21], s_output[21]);
    consume(s_HBM[22], s_output[22]);
    consume(s_HBM[23], s_output[23]);
    consume(s_HBM[24], s_output[24]);
    consume(s_HBM[25], s_output[25]);
    consume(s_HBM[26], s_output[26]);
    consume(s_HBM[27], s_output[27]);
    // consume(s_HBM[28], s_output[28]);
    // consume(s_HBM[29], s_output[29]);
    // consume(s_HBM[30], s_output[30]);
    // consume(s_HBM[31], s_output[31]);

    write_results<28>(s_output, out_PLRAM);
}

void load_HBM(const t_axi* HBM_bank, hls::stream<t_axi>& s_HBM) {
    // Scan the HBM bank for several rounds, 
    //   then write the numbers to FIFO
    ...
    for (int iter_num = 0; iter_num < 128; iter_num++) {
        ...
        #pragma HLS pipeline II = 1
            s_HBM.write(HBM_bank[idx]);
        }
    }
}

void consume(hls::stream<t_axi>& s_HBM, hls::stream<float>& s_output) {
    ...
    s_output.write(reg.PQ_code_A_0);
}

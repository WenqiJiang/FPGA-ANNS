...
#pragma HLS array_partition variable=s_centroid_vectors dim=1
    ...
    hls::stream<dist_cell_ID_t> s_partial_cell_dist[PE_NUM_CENTER_DIST_COMP];
#pragma HLS stream variable=s_partial_cell_dist depth=4
#pragma HLS array_partition variable=s_partial_cell_dist dim=1
    hls::stream<id_cell_ID_t> s_partial_cell_id[PE_NUM_CENTER_DIST_COMP];
#pragma HLS stream variable=s_partial_cell_id depth=4
#pragma HLS array_partition variable=s_partial_cell_id dim=1


    // consume_and_write<QUERY_NUM * CENTROIDS_PER_PARTITION>(s_partial_cell_dist, out_PLRAM)
...
// #pragma HLS array_partition variable=s_input_stream dim=1
    ...
    hls::stream<float> s_merge_input_A_dist;
#pragma HLS stream variable=s_merge_input_A_dist depth=8
    hls::stream<float> s_merge_input_A_id;
#pragma HLS stream variable=s_merge_input_A_id depth=8
    hls::stream<float> s_merge_input_B_dist;
#pragma HLS stream variable=s_merge_input_B_dist depth=8
    hls::stream<float> s_merge_input_B_id;
#pragma HLS stream variable=s_merge_input_B_id depth=8
    hls::stream<float> s_merge_partial_output_dist;
#pragma HLS stream variable=s_merge_partial_output_dist depth=8
    hls::stream<float> s_merge_partial_output_id;
#pragma HLS stream variable=s_merge_partial_output_id depth=8
    hls::stream<float> s_merge_output_dist;
#pragma HLS stream variable=s_merge_output_dist depth=8
    hls::stream<float> s_merge_output_id;
#pragma HLS stream variable=s_merge_output_id depth=8
    ...
    compute_cell_distance_wrapper<QUERY_NUM>(
        s_centroid_vectors, s_query_vectors, s_partial_cell_dist, s_partial_cell_id);

    // broadcast_stream<NLIST, PE_NUM_CENTER_DIST_COMP>(table_DDR0, s_input_stream);

    bitonic_sort(s_partial_cell_dist, s_partial_cell_id, s_merge_input_A_dist, s_merge_input_A_id);

    // periodically write large numbers, and write results out
    merge_filter_streams<PE_NUM_CENTER_DIST_COMP>(
        s_merge_input_A_dist, s_merge_input_B_dist, 
        s_merge_input_A_id, s_merge_input_B_id, 
        s_merge_partial_output_dist, s_merge_partial_output_id);
    result_redirect<PE_NUM_CENTER_DIST_COMP>(
        s_merge_partial_output_dist, s_merge_partial_output_id,
        s_merge_input_B_dist, s_merge_input_B_id,
        s_merge_output_dist, s_merge_output_id); 

    write_result<NPROBE * QUERY_NUM>(s_merge_output_dist, s_merge_output_id, out_PLRAM);
}
...
#pragma HLS UNROLL
        compute_cell_distance<QUERY_NUM>(
            ...
            s_centroid_vectors[s], s_query_vectors[s], 
            s_partial_cell_dist[s], s_partial_cell_id[s]);
    }
}
...
#pragma HLS pipeline II=1
            ...
            s_partial_cell_dist.write(out);
            s_partial_cell_id.write(start_cell_ID + c);
        }
    }
}
...
#pragma HLS pipeline II=1
        ...
        reg_dist0 = input_array_A_dist[count0];
        reg_dist1 = input_array_B_dist[count1];
        reg_id0 = input_array_A_id[count0];
        reg_id1 = input_array_B_id[count1];

        if (reg_dist0 <= reg_dist1) {
            s_merge_partial_output_dist.write(reg_dist0);
            s_merge_partial_output_id.write(reg_id0);
            count0++;
        }
        else {
            s_merge_partial_output_dist.write(reg_dist1);
            s_merge_partial_output_id.write(reg_id1);
            count1++; 
        }
    }
}
...
#pragma HLS dataflow
        ...
        load_stream_to_local<float, input_stream_len>(s_merge_input_A_dist, input_array_A_dist);
        load_stream_to_local<float, input_stream_len>(s_merge_input_B_dist, input_array_B_dist);
        load_stream_to_local<int, input_stream_len>(s_merge_input_A_id, input_array_A_id);
        load_stream_to_local<int, input_stream_len>(s_merge_input_B_id, input_array_B_id);
        merge_filter_arrays<input_stream_len>(
            input_array_A_dist, input_array_B_dist, 
            input_array_A_id, input_array_B_id, 
            s_merge_partial_output_dist, s_merge_partial_output_id);
    }
}
...
void result_redirect(
    ...
    for (int iter = 0; iter < QUERY_NUM; iter++) {
        ...
        for (int i = 0; i < input_stream_len; i++) {
#pragma HLS pipeline II=1
            s_merge_input_B_dist.write(LARGE_NUM);
            s_merge_input_B_id.write(-1);
        }
        ...
        for (int m = 0; m < MERGE_ITER - 1; m++) {
            for (int i = 0; i < input_stream_len; i++) {
#pragma HLS pipeline II=1
                s_merge_input_B_dist.write(s_merge_partial_output_dist.read());
                s_merge_input_B_id.write(s_merge_partial_output_id.read());
            }
        }
        ...
        for (int i = 0; i < input_stream_len; i++) {
#pragma HLS pipeline II=1
            s_merge_output_dist.write(s_merge_partial_output_dist.read());
            s_merge_output_id.write(s_merge_partial_output_id.read());
        }
    }
}
...
void compare_swap(float* array_dist, int* array_id, int idxA, int idxB) {
    // note: idxA must < idxB
#pragma HLS inline
    if (array_dist[idxA] > array_dist[idxB]) {

      	float reg_distA = array_dist[idxA];
        float reg_distB = array_dist[idxB];
      	int reg_idA = array_id[idxA];
        int reg_idB = array_id[idxB];

        array_dist[idxA] = reg_distB;
        array_dist[idxB] = reg_distA;
        array_id[idxA] = reg_idB;
        array_id[idxB] = reg_idA;
    }
}
...
#pragma HLS UNROLL
            compare_swap(array_dist, array_id, 
...
void bitonic_sort(
    ...
    float input_array_dist[32];
    int input_array_id[32];
#pragma HLS array_partition variable=input_array_dist complete
#pragma HLS array_partition variable=input_array_id complete
    // Total: 15 sub-stages
    ...
    for (int iter = 0; iter < QUERY_NUM * MERGE_ITER; iter++) {
        ...
        for (int i = 0; i < 32; i++) {
#pragma HLS UNROLL
            input_array_dist[i] = s_input_stream_dist[i].read();
            input_array_id[i] = s_input_stream_id[i].read();
        }

        // Stage 1
        compare_swap_range_interval<32, 16>(input_array_dist, input_array_id);

        // Stage 2: 2 -> 4
        compare_swap_range_head_tail<32, 8>(input_array_dist, input_array_id);
        compare_swap_range_interval<32, 16>(input_array_dist, input_array_id);

        // Stage 3: 4 -> 8
        compare_swap_range_head_tail<32, 4>(input_array_dist, input_array_id);
        compare_swap_range_interval<32, 8>(input_array_dist, input_array_id);
        compare_swap_range_interval<32, 16>(input_array_dist, input_array_id);

        // Stage 4: 8 -> 16
        compare_swap_range_head_tail<32, 2>(input_array_dist, input_array_id);
        compare_swap_range_interval<32, 4>(input_array_dist, input_array_id);
        compare_swap_range_interval<32, 8>(input_array_dist, input_array_id);
        compare_swap_range_interval<32, 16>(input_array_dist, input_array_id);
        
        // Stage 5: 16 -> 32
        compare_swap_range_head_tail<32, 1>(input_array_dist, input_array_id);
        compare_swap_range_interval<32, 2>(input_array_dist, input_array_id);
        compare_swap_range_interval<32, 4>(input_array_dist, input_array_id);
        compare_swap_range_interval<32, 8>(input_array_dist, input_array_id);
        compare_swap_range_interval<32, 16>(input_array_dist, input_array_id);
        ...
        for (int i = 0; i < 32; i++) {
#pragma HLS pipeline II=1
            s_output_stream_dist.write(input_array_dist[i]);
            s_output_stream_id.write(input_array_id[i]);
        }
    }
}
...
#pragma HLS pipeline II=1
        out_array[i].dist = out_dist.read();
        out_array[i].id = out_id.read();
    }
}
...
//     std::vector<dist_cell_ID_t,aligned_allocator<dist_cell_ID_t>> source_hw_results(size_results_out);
    ...
    int size_results_out_sw = 64 * 32; 
    std::vector<int,aligned_allocator<int>> source_sw_results_ids(size_results_out_sw);
    std::vector<float,aligned_allocator<float>> source_sw_results_dist(size_results_out_sw);

//// INIT SORTED arrays ////

    // DDR0: query vector, QUERY_NUM * D
    int DDR_0_content_size = 10000 * 128 * sizeof(float); 
    
    // DDR1: centroids of single PE, CENTROIDS_PER_PARTITION * D
    int DDR_1_content_size = 8192 * 128 * sizeof(float); 

    int source_sw_results_ids_content_size = size_results_out_sw * sizeof(float);
    int source_sw_results_dist_content_size = size_results_out_sw * sizeof(float);

    char* DDR_0_content_char = (char*) malloc(DDR_0_content_size);
    char* DDR_1_content_char = (char*) malloc(DDR_1_content_size);

    char* source_sw_results_ids_content_char = (char*) malloc(source_sw_results_ids_content_size);
    char* source_sw_results_dist_content_char = (char*) malloc(source_sw_results_dist_content_size);

    std::ifstream DDR_0_content_fstream(
        "/mnt/scratch/wenqi/FPGA-ANNS/saved_npy_data/query_vectors_float32_10000_128_raw", 
        std::ios::in | std::ios::binary);
    std::ifstream DDR_1_content_fstream(
        "/mnt/scratch/wenqi/FPGA-ANNS/saved_npy_data/coarse_centers_float32_8192_128_raw", 
        std::ios::in | std::ios::binary);

    std::ifstream source_sw_results_ids_content_fstream(
        "/mnt/scratch/wenqi/FPGA-ANNS/saved_npy_data/debug_searched_cell_ids_int32_batch_size_64_nprobe_32_raw", 
        std::ios::in | std::ios::binary);
    std::ifstream source_sw_results_dist_content_fstream(
        "/mnt/scratch/wenqi/FPGA-ANNS/saved_npy_data/debug_searched_cell_dist_float32_batch_size_64_nprobe_32_raw", 
        std::ios::in | std::ios::binary);
        

    DDR_0_content_fstream.read(DDR_0_content_char, DDR_0_content_size);
    DDR_1_content_fstream.read(DDR_1_content_char, DDR_1_content_size);

    source_sw_results_ids_content_fstream.read(source_sw_results_ids_content_char, source_sw_results_ids_content_size);
    source_sw_results_dist_content_fstream.read(source_sw_results_dist_content_char, source_sw_results_dist_content_size);

    std::cout << "DDR_0_content_fstream read bytes: " << DDR_0_content_fstream.gcount() << std::endl;
    std::cout << "DDR_1_content_fstream read bytes: " << DDR_1_content_fstream.gcount() << std::endl;

    std::cout << "source_sw_results_ids_content_fstream read bytes: " << source_sw_results_ids_content_fstream.gcount() << std::endl;
    std::cout << "source_sw_results_dist_content_fstream read bytes: " << source_sw_results_dist_content_fstream.gcount() << std::endl;
    ...
    //     printf("\nquery_id: %d\n", i);
    //     for (int j = 0; j < D; j++) {
    //         printf("\t%f", DDR_embedding0[i * D + j]);
    //     }
    // }
    // printf("\nDDR_embedding0\n");

    // memcpy(&HBM_embedding0[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding1[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding2[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding3[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding4[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding5[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding6[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding7[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding8[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding9[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding10[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding11[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding12[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding13[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding14[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding15[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding16[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding17[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding18[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding19[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding20[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding21[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding22[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding23[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding24[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding25[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding26[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding27[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding28[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding29[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding30[0], DDR_0_content_char, 100000*4);
    // memcpy(&HBM_embedding31[0], DDR_0_content_char, 100000*4);
    // for (int i = 0; i < DDR_BANK0_SIZE/*10000 * 128*/; i++) {
    //     DDR_embedding0[i] = 0;
    // }

    memcpy(&DDR_embedding1[0], DDR_1_content_char, DDR_1_content_size);
    // for (int i = 0; i < 8192 * 128; i++) {
    //     DDR_embedding1[i] = 0;
    // }

    memcpy(&source_sw_results_ids[0], source_sw_results_ids_content_char, source_sw_results_ids_content_size);
    memcpy(&source_sw_results_dist[0], source_sw_results_dist_content_char, source_sw_results_dist_content_size);

    free(DDR_0_content_char);
    free(DDR_1_content_char);

    free(source_sw_results_ids_content_char);
    free(source_sw_results_dist_content_char);
...
// OPENCL HOST CODE AREA END
    ...
    std::vector<float> hw_results_dist(NLIST);
    for (int i = 0 ; i < size_results_out; i++) {
        ap_uint<32> tmp0 = source_hw_results[i].range(31, 0);
        ap_uint<32> tmp1 = source_hw_results[i].range(63, 32);
        float dist_local = *((float*) (&tmp0));
        float cell_ID_local = *((int*) (&tmp1));
        std::cout << "i = " << i << "\tdist = " << dist_local
            << "\tcell_ID = " << cell_ID_local << std::endl;
        hw_results_dist[i] = dist_local;

        // std::cout << "i = " << i << "\tdist = " << source_hw_results[i].dist
        //     << "\tcell_ID = " << source_hw_results[i].cell_ID << std::endl;
        // hw_results_dist[i] = source_hw_results[i].dist;
    }
#include <algorithm>
    std::sort(hw_results_dist.begin(), hw_results_dist.end());
    for (int i = 0; i < NPROBE; i++) {
        std::cout << "i = " << i << "\thw dist = " << hw_results_dist[i] <<
            "\tsw dist = " << source_sw_results_dist[i] << "\tdelta = " <<
            hw_results_dist[i] - source_sw_results_dist[i] << std::endl;
    }
    
//     for (int query_id = 0; query_id < QUERY_NUM; query_id++) {
//         for (int nprobe_id = 0; nprobe_id < NPROBE; nprobe_id++) {
//             ap_uint<32> tmp0 = source_hw_results[query_id * NPROBE + nprobe_id].range(31, 0);
//             ap_uint<32> tmp1 = source_hw_results[query_id * NPROBE + nprobe_id].range(63, 32);
//             float dist_local = *((float*) (&tmp0));
//             float cell_ID_local = *((int*) (&tmp1));
//             std::cout << "query_id = " << query_id 
//                 << "\tnprobe_id = " << nprobe_id 
//                 << "\tsw result = " << source_sw_results_dist[query_id * NPROBE + nprobe_id] 
//                 << "\thw result = " << dist_local
//                 << "\tsw ID = " << source_sw_results_ids[query_id * NPROBE + nprobe_id] 
//                 << "\thw ID = " << cell_ID_local
//                 << "\t\t" << 
//                     bool((source_sw_results_dist[query_id * NPROBE + nprobe_id] == dist_local) &&
//                     (source_sw_results_ids[query_id * NPROBE + nprobe_id]  == cell_ID_local))
//                 << std::endl;
//         }
//     }
...
//                 << "\tnprobe_id = " << nprobe_id 
//                 << "\tsw result = " << source_sw_results_dist[query_id * NPROBE + nprobe_id] 
//                 << "\thw result = " << source_hw_results[query_id * NPROBE + nprobe_id].dist
//                 << "\tsw ID = " << source_sw_results_ids[query_id * NPROBE + nprobe_id] 
//                 << "\thw ID = " << source_hw_results[query_id * NPROBE + nprobe_id].cell_ID
//                 << "\t\t" << 
//                     bool((source_sw_results_dist[query_id * NPROBE + nprobe_id] == 
//                         source_hw_results[query_id * NPROBE + nprobe_id].dist) &&
//                     (source_sw_results_ids[query_id * NPROBE + nprobe_id]  == 
//                         source_hw_results[query_id * NPROBE + nprobe_id].cell_ID))
//                 << std::endl;
//         }
//     }

...
void consume_scan_controller_stage_A_1_per_cell(
    ...
    volatile int guard = 0;
    while (
        s_start_addr_every_cell.empty() &
        s_scanned_entries_every_cell_Load_unit.empty() &
        s_scanned_entries_every_cell_Split_unit.empty() &
        s_scanned_entries_every_cell_PQ_lookup_computation.empty() &
        s_scanned_entries_every_cell_Dummy.empty() &
        s_last_valid_channel.empty() 
        ) {
        guard++;
    }
#endif
    printf("consume_scan_controller_stage_A_1_per_cell starts\n");
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
        ...
        int cnt_7 = 0;
        for (int nprobe_id = 0; nprobe_id < NPROBE; nprobe_id++) {
            printf("consume_scan_controller_stage_A_1_per_cell start query %d nprobe %d\n", query_id, nprobe_id);
            cnt_2 += s_start_addr_every_cell.read();
            cnt_3 += s_scanned_entries_every_cell_Load_unit.read();
            cnt_4 += s_scanned_entries_every_cell_Split_unit.read();
            cnt_5 += s_scanned_entries_every_cell_Dummy.read();
            cnt_6 += s_scanned_entries_every_cell_PQ_lookup_computation.read();
            cnt_7 += s_last_valid_channel.read();
            printf("consume_scan_controller_stage_A_1_per_cell end query %d nprobe %d\n", query_id, nprobe_id);
        }
        ...
        printf("consume_scan_controller_stage_A_1_per_cell end query %d\n", query_id);
    }
}
...
void consume_scan_controller_stage_B(
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
        ...
        int cnt_sum = s_consume_scan_controller_stage_A_per_cell.read();

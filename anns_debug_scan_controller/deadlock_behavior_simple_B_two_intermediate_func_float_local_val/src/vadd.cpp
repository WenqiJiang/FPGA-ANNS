...
void consume_input(
    int query_num,
    hls::stream<int> &s_searched_cell_id_scan_controller,
    hls::stream<float> &s_tuple_results) {
...
#pragma HLS pipeline II=1
        ...
        for (int j = 0; j < PRIORITY_QUEUE_LEN; j++) {
            float raw_output = output_stream.read();
            output[i * PRIORITY_QUEUE_LEN + j] = raw_output;
        }
...
#pragma HLS stream variable=s_control_send depth=512
    ...
    hls::stream<float> s_tuple_results; // the top 10 numbers

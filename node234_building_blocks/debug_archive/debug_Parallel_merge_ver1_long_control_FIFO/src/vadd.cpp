...
#pragma HLS dataflow
    ...
    hls::stream<int> s_scanned_entries_every_cell_Dummy[32];
#pragma HLS stream variable=s_scanned_entries_every_cell_Dummy depth=256
...
#pragma HLS stream variable=s_scanned_entries_per_query_Sort_and_reduction depth=256
...
#pragma HLS stream variable=s_scanned_entries_per_query_Result depth=256
...
#pragma HLS stream variable=s_single_PQ_result depth=2
...
#pragma HLS stream variable=s_sorted_PQ_result depth=2
...
#pragma HLS UNROLL
            dummy_PQ_result_sender<QUERY_NUM, NPROBE>(
                s_scanned_entries_every_cell_Dummy[i * 16 + j], s_single_PQ_result[i][j]);
        }
    }
    ...
    consume_and_write_result<QUERY_NUM>(
        s_sorted_PQ_result, s_scanned_entries_per_query_Result, out_PLRAM);
}
...
#pragma HLS pipeline II=1
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
        ...
        for (int nprobe_id = 0; nprobe_id < nprobe; nprobe_id++) {
            ...
            for (int s = 0; s < 32; s++) {
#pragma HLS UNROLL
                s_scanned_entries_every_cell_Dummy[s].write(scanned_entries_every_cell);
            }

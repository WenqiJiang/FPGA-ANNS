...
#pragma HLS array_partition variable=redirected_sorted_stream complete
    ...
    hls::stream<int> s_finish_control[20];
#pragma HLS stream variable=s_finish_control depth=512
#pragma HLS array_partition variable=s_finish_control complete
...
#pragma HLS UNROLL
    consume_and_write( 
        redirected_sorted_stream[s],
        s_scanned_entries_every_cell_Redirected_sorted_stream[s],
        s_finish_control[s],
        query_num);
    }
    ...
    write_result(s_finish_control, out_PLRAM, query_num);
}
...
void consume_and_write(
    hls::stream<single_PQ_result>& redirected_sorted_stream,
    hls::stream<int>& s_scanned_entries_every_cell_Redirected_sorted_stream,
    hls::stream<int>& s_finish_control,
    const int query_num) {

    for (int query_id = 0; query_id < query_num; query_id++) {

        int scanned_entries_every_cell = s_scanned_entries_every_cell_Redirected_sorted_stream.read();

        for (int entry_id = 0; entry_id < scanned_entries_every_cell; entry_id++) {
#pragma HLS pipeline II=1
            redirected_sorted_stream.read();
        }
    } 

    s_finish_control.write(1);
}
...
void write_result(hls::stream<int> (&output_stream)[20], ap_uint<64>* output,
    const int query_num) {

    // only write the last iteration
    for (int s = 0; s < 20; s++) {
        output_stream[s].read();
    }

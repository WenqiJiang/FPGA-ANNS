...
// #pragma HLS RESOURCE variable=s_scanned_entries_every_cell_Dummy core=FIFO_SRL
    ...
    hls::stream<int> s_scanned_entries_per_query_Priority_queue[20];
...
// #pragma HLS RESOURCE variable=s_scanned_entries_per_query_Priority_queue core=FIFO_SRL
    ...
    hls::stream<single_PQ_result> s_single_PQ_result[20];
...
#pragma HLS UNROLL
        ...
        priority_queue_intermediate[i].insert_wrapper(
            s_scanned_entries_per_query_Priority_queue[i], 
            s_single_PQ_result[i], intermediate_result[i],
            query_num);
    }
    ...
    priority_queue_final.insert_wrapper(
            ...
            merged_intermediate_result, s_output, query_num); 
...
#pragma HLS pipeline II=1
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
        ...
        for (int s = 0; s < 20; s++) {
#pragma HLS UNROLL
            s_scanned_entries_per_query_Priority_queue[s].write(accumulated_scanned_entries_per_query);
        }
    }
}
...
void replicate_s_scanned_entries_every_cell_Dummy(
    hls::stream<int>& s_scanned_entries_every_cell_Dummy,
    hls::stream<int> (&s_scanned_entries_every_cell_Dummy_replicated)[20], 
    const int query_num) {

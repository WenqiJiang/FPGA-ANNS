...
void vadd(  
    ...
    result_t* out_PLRAM,
    int iter_num, int query_num_per_iter
    )
{
...
#pragma HLS INTERFACE s_axilite port=iter_num bundle=control
#pragma HLS INTERFACE s_axilite port=query_num_per_iter bundle=control
...
#pragma HLS RESOURCE variable=s_last_result core=FIFO_SRL
    ...
    load_PQ_codes(HBM_in0, s_raw_input[0], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in1, s_raw_input[1], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in2, s_raw_input[2], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in3, s_raw_input[3], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in4, s_raw_input[4], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in5, s_raw_input[5], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in6, s_raw_input[6], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in7, s_raw_input[7], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in8, s_raw_input[8], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in9, s_raw_input[9], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in10, s_raw_input[10], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in11, s_raw_input[11], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in12, s_raw_input[12], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in13, s_raw_input[13], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in14, s_raw_input[14], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in15, s_raw_input[15], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in16, s_raw_input[16], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in17, s_raw_input[17], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in18, s_raw_input[18], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in19, s_raw_input[19], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in20, s_raw_input[20], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in21, s_raw_input[21], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in22, s_raw_input[22], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in23, s_raw_input[23], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in24, s_raw_input[24], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in25, s_raw_input[25], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in26, s_raw_input[26], iter_num, query_num_per_iter);
    load_PQ_codes(HBM_in27, s_raw_input[27], iter_num, query_num_per_iter);
    // load_PQ_codes(HBM_in28, s_raw_input[28], iter_num, query_num_per_iter);
    // load_PQ_codes(HBM_in29, s_raw_input[29], iter_num, query_num_per_iter);
    // load_PQ_codes(HBM_in30, s_raw_input[30], iter_num, query_num_per_iter);
    // load_PQ_codes(HBM_in31, s_raw_input[31], iter_num, query_num_per_iter); 

    type_conversion_and_split_wrapper(
        s_raw_input, s_single_PQ_0, s_single_PQ_1, s_single_PQ_2,
        iter_num, query_num_per_iter); 
    ...
    merge_result_wrapper(
        s_single_PQ_result_0, s_single_PQ_result_1, s_single_PQ_result_2, s_three_PQ_result,
        iter_num, query_num_per_iter);

    consume_and_write_wrapper(s_three_PQ_result, s_last_result, iter_num, query_num_per_iter);
...
void load_PQ_codes(
    ...
    const int iter_num, const int query_num_per_iter) {
...
void type_conversion_and_split(
    ...
    for (int query_id = 0; query_id < iter_num * query_num_per_iter; query_id++) {
#pragma HLS pipeline II=1
...
#pragma HLS inline
    ...
    for (int i = 0; i < HBM_CHANNEL_NUM; i++) {
#pragma HLS UNROLL
        PQ_lookup_computation(s_single_PQ_0[i], s_single_PQ_result_0[i],
            iter_num, query_num_per_iter);
        PQ_lookup_computation(s_single_PQ_1[i], s_single_PQ_result_1[i],
            iter_num, query_num_per_iter);
        PQ_lookup_computation(s_single_PQ_2[i], s_single_PQ_result_2[i],
            iter_num, query_num_per_iter);
    }
}
...
#pragma HLS pipeline II=1
        ...
        int lookup_idx_0 = PQ_local.PQ_code[0];
        int lookup_idx_1 = PQ_local.PQ_code[1];
        int lookup_idx_2 = PQ_local.PQ_code[2];
        int lookup_idx_3 = PQ_local.PQ_code[3];
        int lookup_idx_4 = PQ_local.PQ_code[4];
        int lookup_idx_5 = PQ_local.PQ_code[5];
        int lookup_idx_6 = PQ_local.PQ_code[6];
        int lookup_idx_7 = PQ_local.PQ_code[7];

        // not adding 256 here, it will overflow
        int lookup_idx_8 = PQ_local.PQ_code[8];// + 256;
        int lookup_idx_9 = PQ_local.PQ_code[9];// + 256;
        int lookup_idx_10 = PQ_local.PQ_code[10];// + 256;
        int lookup_idx_11 = PQ_local.PQ_code[11];// + 256;
        int lookup_idx_12 = PQ_local.PQ_code[12];// + 256;
        int lookup_idx_13 = PQ_local.PQ_code[13];// + 256;
        int lookup_idx_14 = PQ_local.PQ_code[14];// + 256;
        int lookup_idx_15 = PQ_local.PQ_code[15];// + 256;
        ...
        out.dist = 
            ...
            distance_lookup_table_local_0[lookup_idx_8 + 256] + distance_lookup_table_local_1[lookup_idx_9 + 256] + 
            distance_lookup_table_local_2[lookup_idx_10 + 256] + distance_lookup_table_local_3[lookup_idx_11 + 256] + 
            distance_lookup_table_local_4[lookup_idx_12 + 256] + distance_lookup_table_local_5[lookup_idx_13 + 256] + 
            distance_lookup_table_local_6[lookup_idx_14 + 256] + distance_lookup_table_local_7[lookup_idx_15 + 256];
...
#pragma HLS UNROLL
        merge_result(
            s_single_PQ_result_0[i], s_single_PQ_result_1[i], 
            s_single_PQ_result_2[i], s_three_PQ_result[i],
            iter_num, query_num_per_iter);
    }
}
...
#pragma HLS UNROLL
        consume_and_write(
            s_three_PQ_result[i], s_last_result[i], iter_num, query_num_per_iter);
    }
}

...
template<const int scan_per_nprobe>
...
#pragma HLS pipeline II=1
        ...
        if ((entry_id == (scan_per_nprobe - 1)) && (last_element_valid == 0)) {
            out.vec_ID = -1;
            out.dist = LARGE_NUM;
        }
...
#pragma HLS UNROLL 
            ...
            for (int s = 0; s < 64; s++) {
#pragma HLS UNROLL
                scanned_entries_every_cell_replicated[s] = scan_per_nprobe;
            }
...
#pragma HLS UNROLL
                for (int j = 0; j < 16; j++) {
#pragma HLS UNROLL
                    PQ_lookup_computation<scan_per_nprobe>(
                        distance_LUT[i * 16 + j],
                        s_single_PQ[i * 16 + j], 
        ...
        #pragma HLS UNROLL
            ...
            dummy_PQ_result_sender<scan_per_nprobe>(
                s_single_PQ_result[3][15]);
        }
    }
}

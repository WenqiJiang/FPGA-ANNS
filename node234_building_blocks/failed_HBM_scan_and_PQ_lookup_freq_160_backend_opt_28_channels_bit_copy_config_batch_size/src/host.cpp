...
// #define QUERY_NUM (ITER_NUM * QUERY_NUM_PER_ITER) // 64 B * 1024 * 1024 = 64 MB
    ...
    const int query_num_per_iter = 1;//1024 * 1024;
...
//     OCL_CHECK(err, err = krnl_vector_add.setArg(1 + 32, buffer_DDR_embedding1));
    ...
    OCL_CHECK(err, err = krnl_vector_add.setArg(29, iter_num));
    OCL_CHECK(err, err = krnl_vector_add.setArg(30, query_num_per_iter));
//////////////////////////////   TEMPLATE END  //////////////////////////////
// ------------------------------------------------------

...
//////////////////////////////   TEMPLATE END  //////////////////////////////
    ...
    int size_results_out = QUERY_NUM * 16; // 256 bit = 32 bytes = 8 floats per result
    std::vector<ap_uint<512>,aligned_allocator<ap_uint<512>>> source_hw_results(size_results_out);
    std::vector<ap_uint<512>,aligned_allocator<ap_uint<512>>> source_sw_results(size_results_out);

    // NOTE: to use ap_uint<512> cannot assume that float will automatically be aligned to a long
    //   512 bit data type, need to specify which bits are mapped to which numbers, otherwise there
    //   the loaded bit order is different than you expected
    //   std::vector<ap_uint<512>, aligned_allocator<ap_uint<512>>> DDR_embedding0(DDR_embedding0_size, 0);
    for (int i = 0; i < QUERY_NUM; i++) { // 512 bit = 64 byte = 16 floats
        for (int j = 0; j < 8; j++) {
            // int, then float
            int local_i = j;
            float local_f = j;
    ...
    // the following will give you the unexpected values
    // std::vector<float, aligned_allocator<float>> DDR_embedding0(DDR_embedding0_size, 0);
//     for (int i = 0; i < QUERY_NUM; i++) { // 512 bit = 64 byte = 16 floats
//         for (int j = 0; j < 8; j++) {
//             // int, then float
//             int start_idx = i * 16 + j * 2;
//             int local_i = j;
//             float local_f = j;
//             // copy 4 bytes from src to dest
//             memcpy(&DDR_embedding0[start_idx + 0], &local_i, 4);
//             memcpy(&DDR_embedding0[start_idx + 1], &local_f, 4);
//         }
//     } 
...
//         for (int j = 0; j < 8; j++) {
//             int start_idx = QUERY_NUM * 16 + 2 * j;
//             // int, then float
//             int local_i;
//             float local_f;
//             memcpy(&local_i, &source_hw_results[start_idx + 0], 4);
//             memcpy(&local_f, &source_hw_results[start_idx + 1], 4);
//             std::cout << "      int = " << local_i <<  "    float = " << local_f << std::endl;
//         }
//     } 

    // similarly, the ap_uint<512> result from hardware need to be interpreted
    for (int i = 0; i < QUERY_NUM; i++) { // 512 bit = 64 byte = 16 floats
        std::cout << "i = " << i  << std::endl;
        for (int j = 0; j < 8; j++) {
            // int, then float

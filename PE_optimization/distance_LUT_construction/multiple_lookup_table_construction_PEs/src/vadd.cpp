...
#pragma HLS resource variable=s_lookup_table core=FIFO_SRL
    ...
    broadcast_PQ_quantizer(table_HBM2, s_PQ_quantizer_init);
    ...
    ////////////////////     write results      ////////////////////
    write_result(s_lookup_table, table_HBM3);
}
...
#pragma HLS resource variable=sub_quantizer_15 core=RAM_2P_URAM
    ...
    for (int row = 0; row < K; row++) {
        ...
        #pragma HLS pipeline II=1
            sub_quantizer_0[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
        int start_addr_1 = row * D + 1 * (D / M);
        for (int col = 0; col < (D / M); col++) {
        #pragma HLS pipeline II=1
            sub_quantizer_1[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
        int start_addr_2 = row * D + 2 * (D / M);
        for (int col = 0; col < (D / M); col++) {
        #pragma HLS pipeline II=1
            sub_quantizer_2[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
        int start_addr_3 = row * D + 3 * (D / M);
        for (int col = 0; col < (D / M); col++) {
        #pragma HLS pipeline II=1
            sub_quantizer_3[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
        int start_addr_4 = row * D + 4 * (D / M);
        for (int col = 0; col < (D / M); col++) {
        #pragma HLS pipeline II=1
            sub_quantizer_4[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
        int start_addr_5 = row * D + 5 * (D / M);
        for (int col = 0; col < (D / M); col++) {
        #pragma HLS pipeline II=1
            sub_quantizer_5[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
        int start_addr_6 = row * D + 6 * (D / M);
        for (int col = 0; col < (D / M); col++) {
        #pragma HLS pipeline II=1
            sub_quantizer_6[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
        int start_addr_7 = row * D + 7 * (D / M);
        for (int col = 0; col < (D / M); col++) {
        #pragma HLS pipeline II=1
            sub_quantizer_7[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
        int start_addr_8 = row * D + 8 * (D / M);
        for (int col = 0; col < (D / M); col++) {
        #pragma HLS pipeline II=1
            sub_quantizer_8[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
        int start_addr_9 = row * D + 9 * (D / M);
        for (int col = 0; col < (D / M); col++) {
        #pragma HLS pipeline II=1
            sub_quantizer_9[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
        int start_addr_10 = row * D + 10 * (D / M);
        for (int col = 0; col < (D / M); col++) {
        #pragma HLS pipeline II=1
            sub_quantizer_10[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
        int start_addr_11 = row * D + 11 * (D / M);
        for (int col = 0; col < (D / M); col++) {
        #pragma HLS pipeline II=1
            sub_quantizer_11[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
        int start_addr_12 = row * D + 12 * (D / M);
        for (int col = 0; col < (D / M); col++) {
        #pragma HLS pipeline II=1
            sub_quantizer_12[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
        int start_addr_13 = row * D + 13 * (D / M);
        for (int col = 0; col < (D / M); col++) {
        #pragma HLS pipeline II=1
            sub_quantizer_13[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
        int start_addr_14 = row * D + 14 * (D / M);
        for (int col = 0; col < (D / M); col++) {
        #pragma HLS pipeline II=1
            sub_quantizer_14[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
        int start_addr_15 = row * D + 15 * (D / M);
        for (int col = 0; col < (D / M); col++) {
        #pragma HLS pipeline II=1
            sub_quantizer_15[row * (D / M) + col] = s_PQ_quantizer_init.read();
        }
    }

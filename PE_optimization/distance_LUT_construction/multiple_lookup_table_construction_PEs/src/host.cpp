...
//         for (int j = 0; j < D; j++) {
//             DDR_embedding0[i * D + j] = query_vec[j];
//         }
//     }
...
//         for (int j = 0; j < D; j++) {
//             DDR_embedding1[i * D + j] = center_vec[j];
//         }
//     }

    // Correct Result: Total distance:  637128
...
//     DDR_embedding1Ext.flags = bank[1 + 32];
    ...
    sourcce_hw_resultsExt.flags = bank[3]; // HBM 3
...
//     OCL_CHECK(err, err = krnl_vector_add.setArg(1 + 32, buffer_DDR_embedding1));
    ...
    OCL_CHECK(err, err = krnl_vector_add.setArg(3, buffer_output));
//////////////////////////////   TEMPLATE END  //////////////////////////////
// ------------------------------------------------------
    ...
    for (int query_id = 0 ; query_id < QUERY_NUM; query_id++) {
        for (int nprobe_id = 0; nprobe_id < NPROBE; nprobe_id++) {
            for (int row = 0; row < K; row++) {
                ...
                for (int j = 0; j < 16; j++) {
                    ap_uint<32> tmp = source_hw_results[result_adddr].range((j + 1) * 32 - 1, j * 32);
                    float dist = *((float*) (&tmp));
                    printf("query_id %d\tnprobe_id %d\tdist_table(%d, %d): %f\n", query_id, nprobe_id, row, j, dist);
                }
            }
        }
    }

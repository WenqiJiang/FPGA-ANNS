...
void vadd(  
    ...
    ap_uint<64> out_PLRAM[QUERY_NUM * CENTROIDS_PER_PARTITION]
    )
{
...
void consume_and_write(
    hls::stream<dist_cell_ID_t> (&s_result)[PE_NUM_CENTER_DIST_COMP], ap_uint<64>* results_out) {
...
#pragma HLS pipeline II=1
    ...
    for (int s = 0; s < PE_NUM_CENTER_DIST_COMP; s++) {
        for (int c = 0; c < CENTROIDS_PER_PARTITION; c++) {
#pragma HLS pipeline II=1
            dist_cell_ID_t tmp = s_result[s].read();
            float dist = tmp.dist;
            int cell_ID = tmp.cell_ID;
            results_out[s * CENTROIDS_PER_PARTITION + c].range(31, 0) = *((ap_uint<32>*) (&dist));
            results_out[s * CENTROIDS_PER_PARTITION + c].range(63, 32) = *((ap_uint<32>*) (&cell_ID));
        }
    }
}

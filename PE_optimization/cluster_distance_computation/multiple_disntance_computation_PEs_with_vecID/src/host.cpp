...
#include <algorithm>
    ...
    float* dist_array = (float*) malloc(8192 * sizeof(float));
    for (int i = 0; i < 8192; i++) {
        ap_uint<32> tmp0 = source_hw_results[i].range(31, 0);
        float dist_local = *((float*) (&tmp0));
        dist_array[i] = dist_local;
    }
    FILE* file = fopen( "dist_array_float32_8192_raw", "wb" );
    fwrite(dist_array, sizeof(float), 8192, file);
    free(dist_array);

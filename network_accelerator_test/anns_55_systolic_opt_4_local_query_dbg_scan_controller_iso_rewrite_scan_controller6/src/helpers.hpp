...
template<const int query_num, const int nlist, const int nprobe>
void scan_controller_per_query(
    hls::stream<int> &s_scanned_entries_every_cell,
    hls::stream<int> &s_scanned_entries_per_query_Sort_and_reduction,
    hls::stream<int> &s_scanned_entries_per_query_Priority_queue) {
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
        ...
        int accumulated_scanned_entries_per_query_buf[nprobe];
        for (int nprobe_id = 0; nprobe_id < nprobe; nprobe_id++) {
            accumulated_scanned_entries_per_query_buf[nprobe_id] = 
                s_scanned_entries_every_cell.read();
        }

        int accumulated_scanned_entries_per_query = 0;
        for (int nprobe_id = 0; nprobe_id < nprobe; nprobe_id++) {
            accumulated_scanned_entries_per_query += 
                s_scanned_entries_every_cell.read();
        }
...
#pragma HLS stream variable=s_scanned_entries_every_cell depth=512
    ...
    scan_controller_per_cell<query_num, nlist, nprobe>(
        HBM_info_start_addr_and_scanned_entries_every_cell_and_last_element_valid,
        s_scanned_cell_id_Input, // from the cluster selection unit
    ...
    scan_controller_per_query<query_num, nlist, nprobe>(
        s_scanned_entries_every_cell,

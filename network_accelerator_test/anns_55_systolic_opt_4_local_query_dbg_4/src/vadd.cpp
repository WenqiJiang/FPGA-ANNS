...
void consume_stage_4(
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
        ...
        distance_LUT_PQ16_t tmp[NPROBE];
        ...
        for (int nprobe_id = 0; nprobe_id < NPROBE; nprobe_id++) {
            ...
            for (int k = 0; k < K; k++) {
#pragma HLS pipeline II=1
                tmp[k] = s_distance_LUT.read();
            }
        }

        for (int w_iter = 0; w_iter < PRIORITY_QUEUE_LEN; w_iter++) {
            out[w_iter].dist = tmp[w_iter].dist_0;
            s_output.write(out[w_iter]);
        }
    }
}

void consume_s_searched_cell_id_scan_controller(
    int query_num,
...
#pragma HLS pipeline II=1
            s_searched_cell_id_scan_controller.read();
        }
    }
}
...
// #pragma HLS resource variable=s_searched_cell_id_scan_controller core=FIFO_BRAM
    ...
    consume_s_searched_cell_id_scan_controller(
        QUERY_NUM,
        s_searched_cell_id_scan_controller);

...
void replicate_s_scanned_entries_every_cell_Dummy(
    hls::stream<int>& s_scanned_entries_every_cell_Dummy,
    hls::stream<int> (&s_scanned_entries_every_cell_Dummy_replicated)[15]) {
...
// #pragma HLS RESOURCE variable=s_scanned_entries_every_cell_PQ_lookup_computation_replicated core=FIFO_SRL
    ...
    hls::stream<int> s_scanned_entries_every_cell_Dummy_replicated[15]; // 48 - 3 * 11 = 15
...
#pragma HLS UNROLL
    ...
    PQ_lookup_computation<query_num, nprobe>(
        // input streams
        s_distance_LUT_systolic[32 + 0 - 1],
        s_single_PQ[32 + 0],
        s_scanned_entries_every_cell_PQ_lookup_computation_replicated[32 + 0],
        s_last_element_valid_PQ_lookup_computation[32 + 0], 
        // output streams
        s_distance_LUT_systolic[32 + 0],
        s_single_PQ_result[2][0]);

    // consume the systolic output of the last PE
    dummy_distance_LUT_consumer<query_num, nprobe>(
        s_distance_LUT_systolic[32]);

    for (int j = 1; j < 16; j++) {
#pragma HLS UNROLL
        dummy_PQ_result_sender<query_num, nprobe>(
            s_scanned_entries_every_cell_Dummy_replicated[j - 1], s_single_PQ_result[2][j]);
    }
}
